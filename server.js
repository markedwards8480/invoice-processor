...
app.get('/api/config', async (req, res) => {
  try {
    const config = {
      ...
      workdriveProcessedFolderId: process.env.WORKDRIVE_PROCESSED_FOLDER_ID || '',
      workdriveFailedFolderId: process.env.WORKDRIVE_FAILED_FOLDER_ID || ''
    };
...
app.post('/api/zoho/bill', async (req, res) => {
  try {
    ...
    const response = await fetch(
      `${apiDomain}/books/v3/bills?organization_id=${organizationId}`,
      {
        method: 'POST',
        headers: {
          'Authorization': `Zoho-oauthtoken ${accessToken}`,
